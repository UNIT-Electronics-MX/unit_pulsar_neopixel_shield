...
#define PIN        8
#define NUM_LEDS   256  // 256 LEDs de matriz (sin offset)
...
int getPixelIndex(int x, int y) {
  ...
  return index; // Sin sumar +1 (sin offset)
}
...
const uint8_t spriteFantasma[8] = {
  ...
  0b11111111,  // Fila 5
  0b11111111,  // Fila 6: Dientes
  0b11011011   // Fila 7: Dientes
};

// Nota Musical (8x8 píxeles)
const uint8_t spriteNota[8] = {
  0b01100000,  // Fila 0: Cabeza nota
  0b01100000,  // Fila 1: Cabeza nota
  0b01100000,  // Fila 2: Cabeza nota
  0b01100000,  // Fila 3: Vástago
  0b11110000,  // Fila 4: Vástago doble
  0b11110000,  // Fila 5: Vástago doble
  0b01100000,  // Fila 6: Conexión
  0b00000000   // Fila 7
};

// ============== FUNCIONES DE DIBUJO ==============

// Función para dibujar un sprite genérico centrado
...
// Función para dibujar Pokébola con colores
    ...
    for (int col = 0; col < 8; col++) {
      if (rowData & (1 << (7 - col))) {
        ...
        if (x >= 0 && x < MATRIX_WIDTH && y >= 0 && y < MATRIX_HEIGHT) {
          int pixelIndex = getPixelIndex(x, y);
          if (pixelIndex >= 0) {
            // Colorear según sección de la pokébola
            if (row < 4) {
              // Parte superior: roja
...
// Función para dibujar Hongo con colores
    ...
    for (int col = 0; col < 8; col++) {
      if (rowData & (1 << (7 - col))) {
        ...
        if (x >= 0 && x < MATRIX_WIDTH && y >= 0 && y < MATRIX_HEIGHT) {
          int pixelIndex = getPixelIndex(x, y);
          if (pixelIndex >= 0) {
            // Colorear según sección del hongo
            if (col < 2) {
              // Sombrero superior: rojo
              strip.setPixelColor(pixelIndex, escalarColor(strip.Color(255, 0, 0)));
            }
  ...
  for (int col = 0; col < 5; col++) {
    ...
    for (int row = 0; row < 8; row++) {
      if (columnData & (1 << row)) {
        ...
        if (pixelIndex >= 0 && pixelIndex < 256) {  // Verificar rango de matriz (LEDs 0-255)
          strip.setPixelColor(pixelIndex, scaledColor);
        }
      }
    }
...
void fuegoIndividual(uint32_t color) {
    ...
    for (int trail = 0; trail < 3; trail++) {
      ...
      if (trailX >= 0 && trailX < MATRIX_WIDTH && 
        ...
        if (pixelIndex >= 0) {
          ...
          if (trail == 0) {
            strip.setPixelColor(pixelIndex, escalarColor(color)); // Punto principal con escalado
          } else {
  ...
  // Fase 2: Explosión
  for (int explodeRadius = 0; explodeRadius <= 4; explodeRadius++) {
    strip.clear();
    
    for (int dx_exp = -explodeRadius; dx_exp <= explodeRadius; dx_exp++) {
      for (int dy_exp = -explodeRadius; dy_exp <= explodeRadius; dy_exp++) {
        if (abs(dx_exp) + abs(dy_exp) <= explodeRadius) {
          int explodePixelX = targetX + dx_exp;
          int explodePixelY = targetY + dy_exp;
...
void testCuatroPuntos() {
  ...
  // LED 0 - Primera posición de fila 0 - ROJO
  strip.setPixelColor(0, strip.Color(255, 0, 0));
  
  // LED 7 - Última posición de fila 0 - VERDE
  strip.setPixelColor(7, strip.Color(0, 255, 0));
  
  // LED 8 - Primera posición de fila 1 (zigzag derecha) - AZUL
  strip.setPixelColor(8, strip.Color(0, 0, 255));
  
  // LED 14 - Penúltima posición de fila 1 (zigzag) - AMARILLO
  strip.setPixelColor(14, strip.Color(255, 255, 0));
  
  // LED 15 - Última posición de fila 1 (zigzag izquierda) - BLANCO
  strip.setPixelColor(15, strip.Color(255, 255, 255));
  
  strip.show();
  
  Serial.println("=== TEST 5 PUNTOS CLAVE (SIN OFFSET) ===");
  Serial.println("LED 0 - Fila 0, inicio: ROJO");
  Serial.println("LED 7 - Fila 0, final: VERDE");
  Serial.println("LED 8 - Fila 1, inicio (der): AZUL");
  Serial.println("LED 14 - Fila 1, penultimo: AMARILLO");
  Serial.println("LED 15 - Fila 1, final (izq): BLANCO");
  Serial.println("");
...
void setup() {
  ...
  Serial.println("Matriz NeoPixel 8x32 iniciada (ZIGZAG - SIN OFFSET)");
  Serial.println("Matriz: LEDs 0-255 en patron serpentina");
